...
export const addProductToBasketAsync = createAsyncThunk("basket/addProductToBasketAsync", async ({ product, count, user }: IAddToBasket, { getState }) => {
    ...
    if (user) {
        const res = await axios.post(`https://frail-frog-leather-jacket.cyclic.app/basket/${user._id}`, product);

        await axios.put(`https://frail-frog-leather-jacket.cyclic.app/basket/${user._id}`, {
            id: product.id,
            count: count
        });
...
export const removeFromBasketAsync = createAsyncThunk("basket/removeFromBasketAsync", async ({ id, user }: IRemoveBasket) => {
    await wait(1000)
    await axios.delete(`https://frail-frog-leather-jacket.cyclic.app/basket/${user._id}/${id}`);
    return id;
})
...
export const getAllBasketItemsAsync = createAsyncThunk("basket/getAllBasketItemsAsync", async () => {
    ...
    const res = await axios.get(`https://frail-frog-leather-jacket.cyclic.app/basket/${user._id}`);
    return res.data;
})
...
export const removeAllItemsInBasketAsync = createAsyncThunk("basket/removeAllItemsInBasketAsync", async (user: IUser) => {
    await axios.delete(`https://frail-frog-leather-jacket.cyclic.app/basket/${user._id}`);
})
